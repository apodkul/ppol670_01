...
```{r}
library(dplyr)
census_data <- read.csv('https://raw.githubusercontent.com/apodkul/ppol670_01/main/Labs/Week02/census_demo.csv')
...
### Joining Data Frames
```{r, eval = F}
census_regions <- read.csv('census_regions.csv')
...
census_data %>% 
  select(STNAME, CTYNAME, hispanic_perc) %>%
  left_join(y = census_regions, by = c('STNAME' = 'State.Name'))
...
test_data_w <- test_data %>% 
  pivot_longer(cols = TOT_POP:TOT_FEMALE, names_to = "var_name")

#Going back (long to wide)
test_data_l <- test_data_w %>%
  pivot_wider(id_cols = STNAME:CTYNAME, names_from = var_name, values_from = value)
```
